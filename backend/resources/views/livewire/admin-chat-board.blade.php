...
>
    ...
    <div x-ref="chatBody" @scroll.passive="handleScroll()" class="flex-grow overflow-y-auto p-5" id="chat-body-{{ $this->getId() }}">
        ...
        @foreach($this->messages as $msg)
            ...
            @endphp
            <div class="flex w-full mb-4 {{ $isAdmin ? 'justify-end' : 'justify-start' }} group relative">
                ...
                <div class="flex flex-col max-w-[75%]">
                    <div class="flex items-center gap-2 {{ $isAdmin ? 'flex-row-reverse' : 'flex-row' }}">
                        ...
                        @if($editingMessageId === $msg->id)
                            <div class="flex flex-col gap-2 bg-white p-3 shadow-md border border-gray-200 w-[260px] rounded-sm">
                        ...
                        @if($isAdmin && $editingMessageId !== $msg->id)
                            <div class="opacity-0 group-hover:opacity-100 transition-opacity flex {{ $isAdmin ? 'flex-row-reverse mr-2' : 'ml-2' }} gap-1">
                                <button wire:click="startEdit({{ $msg->id }}, '{{ addslashes($msg->message) }}')" class="p-1.5 text-gray-400 hover:text-blue-500 bg-white/80 hover:bg-blue-50 shadow-sm transition-colors rounded-sm" title="Edit Pesan">
                                ...
                                <button wire:click="deleteMessage({{ $msg->id }})" onclick="return confirm('Hapus pesan ini?')" class="p-1.5 text-gray-400 hover:text-red-500 bg-white/80 hover:bg-red-50 shadow-sm transition-colors rounded-sm" title="Hapus Pesan">
